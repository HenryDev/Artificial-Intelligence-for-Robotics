...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    omega = [[0 for _ in range(grid_size)] for _ in range(grid_size)]
    xi = [[0] for _ in range(grid_size)]
    ...
    xy_dimensions = range(len(['x', 'y']))
    movement_dimensions = range(len(['x0', 'x1', 'y0', 'y1']))
    measurement_omega = 1 / measurement_noise
    motion_omega = 1 / motion_noise
    for data_row in range(len(data)):
        step = data_row * 2
        observations = data[data_row][0]

        for observation in observations:
            landmark_number = observation[0]
            landmark_index = N * 2 + landmark_number * 2
            for d in xy_dimensions:
                measurement_xi = observation[1 + d] / measurement_noise
                xi[landmark_index + d][0] += measurement_xi
                xi[step + d][0] -= measurement_xi
                omega[step + d][step + d] += measurement_omega
                omega[landmark_index + d][landmark_index + d] += measurement_omega
                omega[step + d][landmark_index + d] -= measurement_omega
                omega[landmark_index + d][step + d] -= measurement_omega

        robot_coordinates = data[data_row][1]
        for d in movement_dimensions:
            omega[step + d][step + d] += motion_omega
        for d in xy_dimensions:
            motion_xi = robot_coordinates[d] / motion_noise
            xi[step + d][0] -= motion_xi
            xi[step + d + 2][0] += motion_xi
            omega[step + d][step + d + 2] -= motion_omega
            omega[step + d + 2][step + d] -= motion_omega
