...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for data_row in data:
        ...
        for i in range(2):
            ...
            omega.value[i][i + 2] += -1 / motion_noise
            omega.value[i + 2][i] += -1 / motion_noise

        a = omega.take([0, 1], a_list)
        b = omega.take([0, 1], [0, 1])
        c = xi.take([0, 1], [0])
        omega_prime = omega.take(a_list)
        xi_prime = xi.take(a_list, [0])
